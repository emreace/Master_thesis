...
vec <- as.numeric(ERA_Rat_iv_PK_properties_reconstructed_v9_geomeans_cleaned_multiple_values_MDR1_pKa_merged_CLpmicros_CLphep_dosemg_ADMET7.4_PKSIM_RH$KineticIV.Clearance..mL.min.kg.)
rango <- ERA_Rat_iv_PK_properties_reconstructed_v9_geomeans_cleaned_multiple_values_MDR1_pKa_merged_CLpmicros_CLphep_dosemg_ADMET7.4_PKSIM_RH
classify <- function(vec) {
  ifelse(vec <= 36.6, "low",
         ifelse(vec > 36.6 & vec < 85.4, "moderate",
                ifelse(vec >= 85.4, "high", NA)))
}
Syst.in.vivo.CLb <- classify(vec)
rango$Syst.in.vivo.CLb <- Syst.in.vivo.CLb
...
for(i in 1:4){
  ...
  x <- as.numeric(tem$KineticEV.F....)
  y <- as.numeric(d$X.Fb_rat.Dose_po_mg) 
  
  
  
  Syst.in.vivo.CLb <- classify(rango$KineticIV.Clearance..mL.min.kg.)
  df <- data.frame(x,y)
  ...
  plot_list <- ggplot(data = df, mapping = aes(x = df$x,y = df$y))+
    ...
    scale_y_continuous(limits = c(0.01,400), labels = comma)+
    scale_x_continuous(limits = c(0.01,400), labels = comma)+
...
for(i in 1:4){
  ...
  x <- as.numeric(tem$KineticEV.AUC..ng.h.mL.)
  y <- as.numeric(d$AUC_rat.Dose_po_mg)
...
for(i in 1:4){
  ...
  x <- as.numeric(tem$KineticEV.Cmax..ng.ml.)
  y <- as.numeric(d$Cmax_rat.Dose_po_mg)
...
for(i in 1:4){
  ...
  x <- as.numeric(tem$KineticEV.Tmax..h.)
  y <- as.numeric(d$Tmax_rat.Dose_po_mg)
