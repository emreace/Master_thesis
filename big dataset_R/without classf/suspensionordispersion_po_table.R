...
dl <- list(P.O.1,P.O.2,P.O.3,P.O.4,P.O.5,P.O.6)

names <- c("P.O.1","P.O.2","P.O.3","P.O.4","P.O.5","P.O.6")
names_vec   <- c("P.O.1","P.O.2","P.O.3","P.O.4","P.O.5","P.O.6")


# Create empty list to store plots
plot_list <- list()
sim_po_F <- matrix(data = NA, nrow = 6, ncol = 2, byrow = FALSE, dimnames = NULL)
for(i in 1:6){
  d <- dl[[i]]
  ...
  df <- data.frame(x,y,z2,z1)
  z22 <- str_detect(z2,"uspension")
  z12 <- str_detect(z1,"uspension")
  z21 <- str_detect(z2,"Dispersion")
  z11 <- str_detect(z1,"Dispersion")
  
  df <- data.frame(x,y,z22,z12,z21,z11)
  df <- subset(df,df$z22==TRUE|df$z12==TRUE|df$z11==TRUE|df$z21==TRUE)
  ...
  sim_po_F[i,] <- c(afe(df$x,df$y), aafe(df$x,df$y))
}
...
# Create empty list to store plots
plot_list <- list()
sim_po_auc <- matrix(data = NA, nrow = 6, ncol = 2, byrow = FALSE, dimnames = NULL)
for(i in 1:6){
  d <- dl[[i]]
  ...
  y <- as.numeric(d$AUC_rat.Dose_mg_p.o)
  z2 <- (d$Formulation.2) 
  z1 <- (d$Formulation) 
  ...
  sim_po_auc[i,] <- c(afe(df$x,df$y), aafe(df$x,df$y))
}
...
# Create empty list to store plots
plot_list <- list()
sim_mat_cmax <- matrix(data = NA, nrow = 6, ncol = 2, byrow = FALSE, dimnames = NULL)
for(i in 1:6){
  d <- dl[[i]]
  ...
  df <- na.omit(df)
  sim_mat_cmax[i,] <- c(afe(df$x,df$y), aafe(df$x,df$y))
  print(paste(max(df),min(df)))
}
...
# Create empty list to store plots
plot_list <- list()
sim_mat_t_max <- matrix(data = NA, nrow = 6, ncol = 2, byrow = FALSE, dimnames = NULL)
for(i in 1:6){
  d <- dl[[i]]
  ...
  df <- na.omit(df)
  sim_mat_t_max[i,] <- c(afe(df$x,df$y), aafe(df$x,df$y))
  print(paste(max(df),min(df)))
}
