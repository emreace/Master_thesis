...
fclb_low_to_moderate <- function(x,y){
  d1 <- as.numeric(data[,x]) *0.015
  d2 <- as.numeric(data[,y])
  d3 <- as.numeric(data$PlasmaProteinBinding.fu.plasma....)>=2
...
fclb_low_to_moderate(x=which(colnames(data)=="KineticIV.Clearance..mL.min.kg."),y = which(colnames(data)=="CL_rat.dose_mg_i.v"))
