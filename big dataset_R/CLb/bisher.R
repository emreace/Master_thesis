...
#setwd("~/Big Dataset/systemic_invivo_CL")
#IVIVE

# Syst.in.vivo.CLb<-optim_RLM_chem_class$KineticIV.Clearance..mL.min.kg.


admed_physiol <- arrange(admed_physiol,admed_physiol$Idorsia.No)
vec <- as.numeric(admed_physiol$KineticIV.Clearance..mL.min.kg.)
...
for(i in 1:6){
  ...
  # df$Syst.in.vivo.CLb <- Syst.in.vivo.CLb
  ...
  # Iterate over names and create plot for each name
    ...
    # print slope
    # print(slope)
    ggsave(plot = plot_list, file = paste(names_vec[i],"Syst.in.vivo.CLb", ".png", sep = ""), width = 10, height = 10)
    # df <- na.omit(df)
