...
PPB <- as.numeric(admed_physiol$PlasmaProteinBinding.fu.plasma....)

PPB <- ifelse(PPB<2,"high","low")
rango <- admed_physiol
# 
...
for(i in 1:6){
  ...
  # Iterate over names and create plot for each name
    ...
    # print slope
    # print(slope)
    ggsave(plot = plot_list, file = paste(names_vec[i],"PPB", ".png", sep = ""), width = 10, height = 10)
    # df <- na.omit(df)
